...
> 下列 DDL 已与当前代码实现对齐，包含开发过程中新增的字段与表，可以按需在数据库中执行或对比。
...
- `IsActive`：标记图书是否“上架”。上架时对前台客户可见并可下单；下架后仅在管理员端“图书管理”中可见与编辑，仍保留库存与历史订单数据。
- `CoverImage`：存储二进制封面图，前端通过 Base64 内联展示。
...
- `CreditLevel`：1–5 级，对应不同折扣与透支能力（在业务逻辑中实现）。
- `MonthlyLimit`：允许的最大透支额度。对 1–2 级客户生效为 0（不可透支），3–5 级允许余额最低为 `-MonthlyLimit`。
- `Balance`：账户当前余额，下单或发货时按信用规则扣减。

------

### 3.6 Orders 与 OrderItem（订单与订单明细）
...
- `CREATED`：订单已创建、尚未扣款（当前实现中一般会直接进入 `PAID`，保留状态）。
- `PAID`：订单已支付成功。
- `PARTIAL`：已部分发货。
- `SHIPPED`：全部发货完成。
- `CANCELLED`：订单已取消。

应用层维护 `TotalAmount` 与 `Status`：

- 创建订单时：按客户 `CreditLevel` 计算折扣价并汇总为 `TotalAmount`，立即进行扣款，成功后状态设为 `PAID`。
- 发货时：根据已发/未发数量更新为 `PARTIAL` 或 `SHIPPED`。
...
- 发货时根据 `Shipment` 累计数量判断是否已完成订单全部发货，从而更新 `Orders.Status`。
- 运单信息（`Carrier`, `TrackingNo`）可用于扩展第三方物流集成。

------

### 3.8 ShortageRecord（缺书记录）
...
CREATE TABLE ShortageRecord (
  ...
  SourceType VARCHAR(50), -- 例如：INVENTORY / ORDER / MANUAL
...
- `SourceType`：标记缺书记录来源（库存预警、订单中缺货、管理员手工登记）。
- `Processed`：是否已通过采购等方式处理。`0=未处理`，`1=已处理`。采购到货后由业务逻辑标记。

------

### 3.9 PurchaseOrder 与 PurchaseItem（采购单与明细）
...
CREATE TABLE PurchaseOrder (
  ...
  FOREIGN KEY (ShortageID) REFERENCES ShortageRecord(ShortageID) ON DELETE SET NULL
) ENGINE=InnoDB;
...
- `ShortageID`：当采购单由“缺书记录 → 生成采购单”创建时，指向对应缺书记录；到货时可精准标记这条缺书记录为已处理。
- `TotalAmount`：按各行采购数量 * 单价在业务逻辑中计算，用于统计报表与首页“待处理采购单”金额。
- `Status`：典型值包括 `CREATED`（已创建，待到货）、`COMPLETED`（已到货）、`CANCELLED` 等。

------

### 3.10 AdminUser（管理员账户）

> 此表为实现登录/权限控制新增，原逻辑设计未包含，现补充。
...
- `Username`：管理员登录名，在 `LoginServlet` 中用于区分管理员和客户。
- `PasswordHash`：管理员密码哈希，安全策略与 `Customer.PasswordHash` 一致。
- `Name`：管理员显示名称，用于界面问候。
